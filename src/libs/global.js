...
export default {
    data: {
        ...
        pageMode: 'club', // 当前页面模式，club--会所网站 9358--公众号
        ...
        defaultCommentsHeader: imgPathPrefix + 'defaultCommentLogo.png', // 评论列表默认头像
    ...
    beforeInit () {
        ...
        ua.isAndroid = /Android/i.test(uaStr)
        ...
        // 获取clubID
        let tArr = location.href.toString().match(/\/(\d{18,24})\/?/)
        if (tArr && tArr[1]) {
            data.pageMode = 'club'
            data.clubId = tArr[1]
        } else {
    ...
    // 查询各种券的提醒数量 // 世界杯的提现消息数
    queryReminds () {
        let data = this.data
        let reminds = data.reminds
        if (data.isLogin) {
            Vue.http.get('../api/v2/user/worldcup/menu/inlet', {
                params: {
                    clubId: data.clubId
                }
            }).then(resCup => {
                resCup = resCup.body
                if (resCup.statusCode == 200) {
                    resCup = resCup.respData
                    reminds.worldCupInlet = resCup.inlet
                }
            })
     ...
     * @param {int} value 选填 用于填写打分型事件的分值，加载时间型事件的时长，订单型事件的价格。（请填写整数数值，如果填写为其他形式，系统将按0处理。若填写为浮点小数，系统会自动取整，去掉小数点。）
     * @param {String} nodeid 填写事件元素的div元素id。（请填写class id，暂不支持name。）
     */
    setUmengStatistics (category, action, label, value, nodeid) {
        window._czc.push(['_trackEvent', category || location.hostname, action || location.href, label, value, nodeid])
    },

    imRefreshToken () {
        let that = this
        let data = that.data
        Vue.http.get('../api/v1/emchat/refresh/password').then(res => {
            res = res.body
            if (res.statusCode == 200) {
                data.chatToken = res.respData
                that.init()
            } else {
                console.log(res.msg || '获取IM token失败')
            }
        })
    },
    ...
    getPayWay (clubId) {
        return new Promise((resolve, reject) => {
            Vue.http.get(Api.PAY_BANK_CODE, {
     ...
     * @param {*} clubId
     */
    getSandHemaWechatAuth (url, clubId) {
        // { params: { clubId: clubId } }
        return new Promise((resolve, reject) => {
            Vue.http.get(Api.SAND_HEMA_WECHAT_AUTH, {
                params: {
                    redirectUrl: url,
                    clubId: clubId
                }
     ...
     * @param {*} qrcodeId
     * @param {*} qrcodeType
     * @param {*} statType appoint|预约 join|活动参与 reward|打赏
     */
    scanCodeStat (clubId, qrcodeId, qrcodeType, statType, techId = '') {
        return new Promise((resolve, reject) => {
            Vue.http.post(Api.CODE_STAT, {
                clubId: clubId,
                qrcodeId: qrcodeId || '',
