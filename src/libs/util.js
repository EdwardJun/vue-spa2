...
export default {
    ...
    tipShow (content, time) {
        ...
        let id = store.state.tipShowTimeoutId
        if (!tip) {
        ...
        store.state.tipShowTimeoutId = setTimeout(() => {
            tip.classList.remove('active')
            tip.classList.remove('show')
            store.state.tipShowTimeoutId = 0
        }, (time || 3000))
    },
     ...
     * @param {*} childList 传递过来的字段是否还要解析一层数组
     */
    sortBy(attr,rev,childList) {
        if(rev ==  undefined){
            rev = 1;
        }else{
            rev = (rev) ? 1 : -1;
        ...
        return (a,b) => {
          if(childList) {
            a = a[childList][0][attr];
            b = b[childList][0][attr];
          }else {
            a = a[attr];
            b = b[attr];
          }
