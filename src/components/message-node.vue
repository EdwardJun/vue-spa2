...
<template>
    <div class="messageNode" :class="messageCls" v-if="!msg.isRevoke">
        ...
        <template v-else>
            ...
            <div v-else-if="type=='ordinaryCoupon'" class="ordinaryCoupon" @click="doClickMsgContent('ordinaryCoupon')"><i>{{ msg.data.typeName }}</i>{{ msg.data.couponName }}<span></span><b>{{ msg.data.validPeriod }}</b></div>
        </template>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            doOrderRequest () {
                let that = this
                let talker = that.talker
                window._czc.push(['_trackEvent', 'chat', '点我预约', '聊天窗口——点我预约', '', ''])
                if (talker.type == 'tech') {
            ...
            doHandlerPayOrder () {
                ...
                // }
                that.$http.post('../api/v2/wx/pay/payment/pay_immediate', paramData).then(res => {
                    res = res.body
