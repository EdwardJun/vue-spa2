...
<template>
    <div class="bind-phone-wrap pop-modal" :class="{ active : show }">
        <div class="center-wrap">
            <div class="wrap">
                <div class="title"><div v-if="!explain">绑定手机</div> <span v-if="explain">{{ explain }}</span></div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            isTelValid () {
                return /^1[23456789]\d{9}$/.test(this.phoneNumber)
            }
        },
        created () {
            var that = this
            eventHub.$on('pop-bind-phone', function (explain) {
