        ...
        methods: {
            ...
            initPage () {
                ...
                that.$http.get('../api/v2/club/order/appoint/view', {params: {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (that.isPayAppointment) { // 付费预约时查询支付方式
            ...
            doConfirmOrder () {
                ...
                if (!that.serviceItem.id && that.appointType == 'paid_full') {
                    Global.setUmengStatistics(that.category, action, '确认订单页面——全额预约时未填写预约项目')
                    return Util.tipShow('请选择预约项目！')
                } else if (that.appointType != 'phone') {
                    if (!that.isCanAppoint) {
                        Global.setUmengStatistics(that.category, action, '确认订单页面——当前无可预约时间！')
                        return Util.tipShow('当前无可预约时间！')
                    }
                    if (!that.currAppointTime.time) {
                        Global.setUmengStatistics(that.category, action, '确认订单页面——未填写到店时间')
                        return Util.tipShow('请选择到店时间！')
                    }
                    if (!that.contactName) {
                        Global.setUmengStatistics(that.category, action, '确认订单页面——未填写联系人姓名')
                ...
                if (!that.oldOrderId) {
                    ...
                    if (that.isPayAppointment && that.downPayment > 0) {
                        ...
                        Global.setUmengStatistics(that.category, '确认预约', '确认订单页面——免费预约')
                        that.$http.post('../api/v2/club/order/appoint', {
                            ...
                            that.inSubmit = false
                            if (res.statusCode == 200) {
                                Global.setUmengStatistics(that.category, '免费预约成功', '确认订单页面')
                                Util.tipShow('预约成功！')
                                // 二维码统计
                                Global.scanCodeStat(that.clubId, that.qrcodeId, that.qrcodeType, 'appoint').then(res => {
                                    // console.log(res)
                                })
                                that.sendMessage(res.respData)
                            } else {
                                Global.setUmengStatistics(that.category, '免费预约失败', '确认订单页面')
                    ...
                    Global.setUmengStatistics(that.category, '确认预约', '确认订单页面，oldOrderId')
                    that.$http.get('../api/v2/club/order/update_time', {params: {
                        ...
                        that.inSubmit = false
                        if (res.statusCode == 200) {
                            Global.setUmengStatistics(that.category, 'oldOrderId预约成功', '确认订单页面，oldOrderId')
                            Util.tipShow('预约成功！')
                            that.sendMessage(that.oldOrderId)
                        } else {
                            Global.setUmengStatistics(that.category, 'oldOrderId预约失败', '确认订单页面，oldOrderId')
                            Util.tipShow(res.msg || '预约请求失败！')
                        }
                    }, function () {
                        Global.setUmengStatistics(that.category, 'oldOrderId预约失败', '确认订单页面，oldOrderId')
            ...
            doPay (dateDay, dateTime, downPayment, serviceDuration) {
                ...
                // sand支付
                let clubId = that.global.clubId ? that.global.clubId + '/' : ''
                let sandSuccessUrl = that.global.prefixPathname + '#/' + clubId + 'order?payType=sandPay'
                ...
                that.$http.post('../api/v2/wx/pay/paid_order', paramData).then(function (res) {
                    res = res.body
                    if (res.statusCode == 200) {
                        Global.setUmengStatistics(that.category, '预约调起支付微信支付：WeixinJSBridge', '确认订单页面，点击确认预约按钮调起支付')
                        var payInfo = JSON.parse(res.respData)
                        if (that.paymentChannel == 'sand') {
                            location.href = payInfo.payUrl
                        } else {
                            Global.wxJsBridgeReady(() => {
                                WeixinJSBridge.invoke('getBrandWCPayRequest', {
                                    ...
                                    if (payRes.err_msg.indexOf('ok') >= 0) { // 支付成功之后
                                        Util.tipShow('支付成功！')
                                        Global.setUmengStatistics(that.category, '预约支付成功', '确认订单页面')
                                        that.sendMessage(payInfo.bizId)
                                        // 二维码统计
                                        Global.scanCodeStat(that.clubId, that.qrcodeId, that.qrcodeType, 'appoint')
                                    } else {
                                        Util.tipShow('未能成功支付！')
                                        Global.setUmengStatistics(that.category, '预约支付失败', '确认订单页面')
