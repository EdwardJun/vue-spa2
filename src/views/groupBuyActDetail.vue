    ...
    <div class="page" id="group-buy-act-detail-page">
        ...
        <div class="pop-modal all-groups-pop" :class="{ active: showAllGroupsModal }">
            <div class="center-wrap">
                <div>正在拼团</div>
                <div>
                    <div class="pop-group-item" v-for="(group) in openGroupList" :key="group.id">
                        <div :style="{ 'background-image': 'url('+(group.avatarUrl || global.defaultHeader)+')' }"></div>
                        <div>
                            <div>{{ group.userName }}</div>
                            <div v-show="group.timeStr">剩余<span>{{ group.timeStr }}</span></div>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.bizId = query.bizId
            that.payType = query.type
            if (!that.actId) {
                Util.tipShow(global.visitError)
                return that.$router.back()
            }
            if (that.actId && that.bizId && that.payType && that.count) {
                that.afterPay(that.bizId)
            }
        ...
        methods: {
            ...
            doClickPayBtn () {
                ...
                if (ua.isWX && that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    location.reload()
