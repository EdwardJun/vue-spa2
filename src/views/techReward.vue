...
<template>
    <div class="page" id="tech-reward-page">
        ...
        <div class="section" v-if="moneySwitchOn">
            ...
            <ul class="items">
                <li class="item" v-for="(item, index) in moneySettingList" @click="clickItem(index,'money',item.amountType)" :key="index">
        ...
        <div class="section" v-if="creditSwitchOn">
            ...
            <ul class="items">
                <li class="item" v-for="(item, index) in creditSettingList" @click="clickItem(index,'credit')" :key="index">
    ...
    export default {
        ...
        methods: {
            ...
            moneyReward (num) {
                ...
                that.$http.post('../api/v2/wx/pay/user_reward', paramData).then(res => {
                    res = res.body
                    if (res.statusCode == 200) {
                        let payInfo = JSON.parse(res.respData)
                        that.prePayId = payInfo.package.split('=')[1]
                        that.doInvokeWxPay(payInfo, that.prePayId, num)
            ...
            doInvokeWxPay (payInfo, prePayId, num) {
                let that = this
