...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.$http.get('../api/v2/club/{clubId}/{actId}/actdetail', {params: {clubId: global.clubId, actId: query.id}}).then(function (res) {
                ...
                if (res.statusCode == 200) {
                    ...
                    that.shareSetting(res.act.shareUrl, res.act.actLogoUrl, res.act.actTitle, res.act.actPeriod)
                } else {
        ...
        methods: {
            ...
            shareSetting (shareUrl, imageUrl, name, desc) {
                var that = this
                var global = that.global
                Global.shareConfig({
                    title: global.clubName + '-' + name,
                    desc: desc,
