...
<template>
    <div class="page" id="service-list-page" v-show="!global.loading" @scroll="doScrollPage()">
        ...
        <div class="category" v-for="(categoryItem,index) in serviceList" :key="index" :class="'c'+index">
            <div>
                <div :style="{ 'background-image' : 'url(' + categoryItem.category.imageUrl + ')' }"></div>
                <div>{{ categoryItem.category.name }}</div>
            </div>
            <ul class="service-list">
                <router-link tag="li" v-for="(item, index) in categoryItem.serviceItemMap.online" :key="index" :to="{ name: 'serviceItem', query: { id : item.id }}">
                    <div :style="{ backgroundImage : 'url('+(item.coverUrl || global.defaultServiceItemImgUrl)+')' }"></div>
                    <div>
                        <div>{{ item.name }}</div>
                        <div v-for="(priceItem, priceIndex) in item.priceList" :key="priceIndex">
                            <div v-for="(unitItem, unitIndex) in priceItem.itemList" :key="unitIndex">
                                {{ unitItem.price/100 | itemPriceFormatter(unitItem.duration || unitItem.amount,unitItem.unitName) }} 
                            </div>
                        </div>
                    </div>
                </router-link>
        ...
        mounted () {
            ...
            that.$http.get('../api/v1/club/category/getCategoryItemList/new', {params: {clubId: global.clubId}}).then(res => {
                ...
                if (res.statusCode == 200) {
                    ...
                    let fixedServiceId = global.currPage.query.id
                    if (fixedServiceId) {
                        var fixedIndex = 0
                        for (let k = 0; k < that.serviceList.length; k++) {
                            if (that.serviceList[k].category.id == fixedServiceId) {
                                fixedIndex = k
                            }
                        }
                        if (fixedIndex != 0) {
