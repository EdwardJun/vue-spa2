    ...
    export default {
        ...
        methods: {
            ...
            submit () { //  微信支付"
                ...
                if (this.paymentChannel == 'sand' && !this.sandOpenId && !this.sandSubOpenId) {
                    location.reload()
                ...
                this.$http.post('../api/v2/wx/pay/item/order/save', paramData).then((res) => {
                    res = res.body
                    if (res.statusCode == 200) {
            ...
            pay () {
                ...
                let sandSuccessUrl = this.global.prefixPathname + '#/' + this.clubId + '/itemCardOrders'
                if (this.paymentChannel == 'sand') {
                    paramData.sandOpenId = this.sandOpenId || ''
                    paramData.sandSubOpenId = this.sandSubOpenId || ''
                    paramData.sandCancelUrl = location.href || '' // 取消支付跳转页
                    paramData.sandSuccessUrl = sandSuccessUrl || '' // 支付成功跳转页
                }
                that.$http.post('../api/v2/wx/pay/item/pay', paramData).then((res) => {
                    res = res.body
                    if (res.statusCode == 200) {
