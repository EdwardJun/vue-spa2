<template>
    <div class="page club-list-page" id="club-list-search-page" :style="{ height : (global.winHeight-2.667*global.winScale*16)+'px' }" @scroll="doHandlerScroll()">
        ...
        <div class="tags" v-show="showTags">
            <div>会所</div>
            <ul class="clear-fix">
                <li v-for="(item, clubIndex) in clubTags" :key="clubIndex" @click="doClickTag(item)">{{ item }}</li>
            </ul>
        </div>
        <!-- 
            <div class="tags" v-show="showTags && techTags.length>0">
                <div>技师</div>
                <ul class="clear-fix">
                    <li @click="doClickTag(item.name)" v-for="(item, techIndex) in techTags" :key="techIndex">{{ item.name }}</li>
                </ul>
            </div>
         -->
