    ...
    <div class="page" id="order-list-page">
        <div class="order-list" ref="listEle" :style="{ height : (global.winHeight-(global.pageMode=='club' ? 2.9: 0)*global.winScale*16)+'px'}" @scroll="doHandlerOrderListScroll()">
            <div class="order-item" v-for="(order,$index) in orderList" :key="order.id">
                ...
                <a class='paid' v-if="order.orderType=='paid' && (order.status=='unpaid'||order.status=='process')" @click="doClickPaid($index)">支付</a>
                ...
                <router-link class='comment' v-if="order.status=='complete' && order.techId && order.commented == 'N' && (isRewardOn && !commentSwitch || order.rewardSwitch && !order.commentSwitch)" :to="{name:'twoCommentReward',query:{techId:order.techId}}">打赏</router-link>
                <a class='refunded' v-if="order.orderType=='paid' && order.status=='refund'" @click="doClickRefundQuery($index)">查询</a>
                <a class='reAppoint' v-if="order.status=='error'" @click="doClickAppointment(order)">预约</a>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.payType = query.payType
            // console.log(that.global.prefixPathname + '#/' + that.global.clubId + '/order')
            // sand支付时，从confirmOrder预约技师项目跳转过来二维码统计
            if (that.payType == 'sandPay') {
                var ss = Util.sessionStorage
                that.qrcodeId = ss('qrcodeId') || ''
                that.qrcodeType = ss('qrcodeType')
                Global.scanCodeStat(that.global.clubId, that.qrcodeId, 'tech_qrcode', 'appoint')
            }
            that.init()
        },
        mounted () {
        ...
        methods: {
            ...
            hemaAuth (data, index) {
                ...
                if (that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    let url = location.href + '?type='
                    Global.getSandHemaWechatAuth(url, data.clubId).then((res) => {
                        location.href = res
                    }, (err) => {
                        Util.tipShow(err)
                    })
            ...
            doClickPaid (index) { // 支付
                ...
                // 获取支付渠道
                Global.getPayWay(data.clubId).then((res) => {
                    that.paymentChannel = res
                    that.hemaAuth(data, index)
                }, function (err) {
                    Util.tipShow(err)
                })
            },
            ...
            paidOrder (data, index) {
                ...
                let sandSuccessUrl = that.global.prefixPathname + '#/' + clubId + 'orderDetail?payType=sandPay&id=' + data.id
                ...
                that.$http.post('../api/v2/wx/pay/paid_order_immediately', paramData).then((result) => {
                    result = result.body
                    if (result.statusCode == 200) {
                        if (that.paymentChannel == 'sand') {
                            let respData = JSON.parse(result.respData)
            ...
            doClickAppointment (order) {
                let that = this
                Global.setUmengStatistics(that.category, '预约', '订单页的预约按钮')
                that.$router.push({
                    name: 'confirmOrder',
                    query: { orderId: order.id, clubId: order.clubId, downPayment: order.downPayment, customerName: encodeURIComponent(order.customerName), itemId: order.itemId, techId: order.techId }
                })
            }
        },
