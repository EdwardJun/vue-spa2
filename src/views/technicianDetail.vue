...
<style lang="sass">
    @import '../sass/page/technicianDetail.scss';
</style>
<template>
    <div>
        <div class="page" id="technician-detail-page" :style="{ 'min-height': global.winHeight + 'px' }" :class="{'no-padding': blacklistModal }">
            <div class="top">
                ...
                <div class="name"><span>{{ techName }}</span><span v-show="techNo">{{ techNo }}</span></div>
                <div class="desc" v-show="techDesc">{{ techDesc }}</div>
                <div class="collect-comment" :class="{ commentSwitch: !commentSwitch }">
                    ...
                    <span v-if="commentSwitch"></span>
                    <router-link v-if="commentSwitch && commentLabelCountList.length>0" :to="{ name : 'commentAll' , query : { techId : techId }}" tag="div" class="comment-btn"><div></div> <div>评论</div> <span>{{ commentLabelCountList[0].count }}</span></router-link>
                </div>
            ...
            <div class="service-item" v-show="serviceItems.length>0 && displaySelectProject">
                ...
                <div class="wrap">
                    <div class="item" v-for="(service, sIndex) in serviceItems" :key="sIndex" @click="doSelectServiceItem(service.id)" v-if="isMore || sIndex<=2">
                        ...
                        <div @click="doClickOrderItemBtn(service)">预约</div>
                    </div>
                </div>
                <div class="item-more" v-if="serviceItems.length > 2" :class="{ active: isMore }" @click="doClickMore()">{{ isMore ? '收起' : '更多' }}</div>
            </div>

            <div class="view" @click="doViewOtherTech()">
                <div class="tip">更多技师</div>
                <div class="other-tech" v-for="(item, oIndex) in otherTechs" :key="oIndex" :style="{'background-image': 'url('+item.avatarUrl+')'}"></div>
            </div>

            <!--评论-->
            <div class="comments-wrap" v-if="commentSwitch">
                <div class="comments-wrap_h" v-if="commentDescription">{{ commentDescription }}</div>
                <ul class="comments-list" v-if="commentList.length>0">
                    <li class="comments-item" v-for="comment in commentList" :key="comment.id">
                        <div class="comments-item_h">
                            <div :style="{ 'background-image': 'url('+ (comment.userInfo.avatarUrl || global.defaultCommentsHeader) +')' }"></div>
                            <div><div>{{ comment.userInfo.name }}</div></div>
                            <div>{{ comment.createdAt | DateFormatter('yyyy-MM-dd hh:mm') }}</div>
                        </div>
                        <div v-if="!comment.onlyReward" class="comments-grade">评分 <div><div :style="{ width: comment.rate + '%' }"></div></div></div>
                        <div v-if="!comment.onlyReward">
                            <div class="comments-item_bd" v-html="comment.comment"></div>
                            <div class="comments-item_ft">
                                <div><label v-if="comment.impression">  {{ comment.impression }}</label><span v-if="comment.integralAmount||comment.rewardAmount">赞赏 <i v-if="comment.rewardType=='integral'">{{ comment.integralAmount }}积分</i><i v-if="comment.rewardType=='money'">￥{{ comment.rewardAmount / 100 }}</i></span></div>
                            </div>
                        </div>
                        <div v-if="comment.onlyReward" class="only-reward">
                            <div><span v-if="comment.integralAmount||comment.rewardAmount">赞赏 <i v-if="comment.rewardType=='integral'">{{ comment.integralAmount }} 积分</i><i v-if="comment.rewardType=='money'">￥{{ comment.rewardAmount / 100 }}</i></span></div>
                        </div>
                    </li>
                </ul>
                <div class="data-load-tip" :class="{ none : !showDataLoadTip }"><div>加载数据</div></div>
                <div class="finish-load-tip" :class="{ none : !showFinishLoadTip }"><router-link tag="div" :to="{ name : 'commentAll', query : { techId: techId, clubId: clubId } }">查看全部评论</router-link></div>
                <div class="no-comments" v-show="commentList.length==0 && !isAddData">暂无评论...</div>
            </div> 
        </div>
        ...
        <div class="tech-detail-footer-wrap" v-show="!blacklistModal">
            <a v-if="canReward" class="tech-reward" @click="doClickRewardBtn()"><i>打赏</i></a>
            <router-link class="tech-im" :to="{ name : 'chat', query : { techId: techId, clubId: clubId } }"><i>聊天</i></router-link>
            <a @click="doClickOrderBtn()" class="tech-appoin"><span class="reward-close" :class="{ active : canOrder, unable: techStatus == 'rest', rewardClose: !canReward }">约<b>Ta</b></span></a>
        </div>
        ...
        <div class="coupon-list" :class="{ active : showCouponList }">
            <div>
                ...
                <div class="list">
                    ...
                    <div class="coupon-item ordinary" v-for="(coupon, index) in ordinaryCoupons" :key="index">
                        <div>
                            <div>{{ coupon.actTitle }}</div>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.sandOpenId = query.open_id
            that.sandSubOpenId = query.sub_open_id
        ...
        methods: {
            hemaAuth () {
                let that = this
                if (that.global.userAgent.isWX && that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    Global.getSandHemaWechatAuth(location.href, that.clubId).then((res) => {
                        location.href = res
                        that.global.userAgent.isAndroid && setTimeout(() => {
                            location.reload()
                        }, 2500)
                    }, (err) => {
                        Util.tipShow(err)
            ...
            init () {
                ...
                that.$http.get('../api/v2/club/technician/{techId}', { params: { techId: query.id } }).then(res => {
                    ...
                    if (res && res.info) {
                        ...
                        // 获取支付渠道
                        Global.getPayWay(that.clubId).then((res) => {
                            that.paymentChannel = res
                            that.hemaAuth()
                        }, function (err) {
                            Util.tipShow(err)
                        })
            ...
            doClickOrderBtn () { // 点击预约按钮
                ...
                // Global.setUmengStatistics('technicianDetail', '约ta', '技师详情页')

                if (that.techStatus == 'rest') {
            ...
            doGetPaidCoupon (coupon, event) { // 点击购买点钟券
                ...
                if (global.userAgent.isWX && that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    location.reload()
                    // return Util.tipShow('缺少杉德支付OpenId！')
                    return console.log('缺少杉德支付OpenId！')
                }
                if (!global.isLogin) {
                    Util.tipShow('请您先登录！')
                    ...
                    let sandSuccessUrl = that.global.prefixPathname + '#/' + that.clubId + '/' + 'technicianDetail?id=' + that.techId
                    if (that.paymentChannel == 'sand') {
                        paramData.sandOpenId = that.sandOpenId || ''
                        paramData.sandSubOpenId = that.sandSubOpenId || ''
                        paramData.sandCancelUrl = location.href || '' // 取消支付跳转页
                        paramData.sandSuccessUrl = sandSuccessUrl || '' // 支付成功跳转页
                    }
                    that.$http.post('../api/v2/wx/pay/paid_coupon', paramData).then(res => {
                        res = res.body
                        if (res.statusCode == 200) {
                            that.payRequestObj = JSON.parse(res.respData)
                            if (that.paymentChannel == 'sand') {
                                location.href = that.payRequestObj.payUrl
                            } else {
            ...
            doHandlerListScroll () {
                ...
                if (that.page <= 2 && !that.isDataAddEnd && scrTop + global.winHeight * 1.4 > body.scrollHeight) {
            ...
            queryCommentList (page) {
                ...
                // '../api/v2/club/technician/comments'
                that.$http.get(Api.COMMENT_EXCELLENCE, {
                    params: {
                        page: page,
                    ...
                    if (res.statusCode == 200) {
                        res = res.respData || []
                        that.commentDescription = res.description
                        that.handlerCommentData(res.list, page)
                    } else {
                        Util.tipShow(global.loadError)
                    }
                }, () => {
