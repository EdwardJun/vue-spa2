    ...
    export default {
        ...
        methods: {
            ...
            shareSetting (shareUrl, imageUrl, name) {
                var that = this
                var global = that.global
                let href = that.global.prefixPathname + '#/' + that.clubId + '/robProjectDetail?payType=sand&robProjectId=' + that.itemId
                Global.shareConfig({
            ...
            doClickPayBtnOfCredit () {
                ...
                if (that.isCredits) {
                    ...
                    that.$http.get('../api/v2/club/credits/paid/service_item', {
                        ...
                        if (res.statusCode == 200) {
                            Util.tipShow('支付成功！')
                            Global.scanCodeStat(that.clubId, that.qrcodeId, that.qrcodeType, 'join').then((res) => {
                                that.$router.push({name: 'robProjectSuccess', query: {bizId: robProjectId, isIntegral: 'true', clubId: that.clubId, techId: that.techId}})
            ...
            doClickPayBtnOfCash () {
                ...
                // TODO 链接需要在后台配置bizId,下个页面需要用到
                let sandSuccessUrl = that.global.prefixPathname + '#/' + that.clubId + '/robProjectSuccess?payType=sand&clubId=' + that.clubId + '&techId=' + that.techId
                ...
                that.$http.post('../api/v2/wx/pay/paid_service_item/save', paramData).then(function (res) {
                    res = res.body
                    if (res.statusCode == 200) {
                        that.cashPayData = JSON.parse(res.respData)
                        if (that.paymentChannel == 'sand') {
                            location.href = that.cashPayData.payUrl
                        } else {
            ...
            onBridgeReady () {
                ...
                WeixinJSBridge.invoke('getBrandWCPayRequest', {
                    ...
                    if (res.err_msg.indexOf('ok') >= 0) {
                        Util.tipShow('支付成功！')
                        Global.scanCodeStat(that.clubId, that.qrcodeId, that.qrcodeType, 'join').then((res) => {})
                        that.$router.push({name: 'robProjectSuccess', query: {bizId: cashPayData.bizId, clubId: that.clubId, techId: that.techId}})
                    } else {
                        that.cashProcessing = false
                        Util.tipShow('未能成功支付！')
                        // 支付失败，删除预支付订单
                        that.payCancel(cashPayData.payId)
                    }
                })
            },
            payCancel (payId) {
                let that = this
                that.$http.get('../api/v2/wx/pay/activity/payment/cancel', {params: {payId: payId}}).then(res => {
                    if (res.statusCode == 200) {
                        that.$router.push({name: 'robProjectDetail', query: {robProjectId: that.global.currPage.query.robProjectId}})
                    }
                })
            },
