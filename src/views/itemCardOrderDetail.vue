...
<template>
    <div class="page" id="item-card-order-detail-page">
        ...
        <div class="can-use list" v-show="currShowIndex==0">
            ...
            <div class="item" v-show="orderData.canUseCount > 0" v-for="(item, cIndex) in canUseList" :key="cIndex" :class="{ expired: orderData.status == 2, active: item.isShow }">
                <h4 @click="doSwitchShow(item)">{{ item.itemName }}券</h4>
                <div class="coupon" v-for="(coupon,index) in item.coupons" :key="index">
                    <div>券{{ index + 1 }}</div>
        ...
        <div class="used list" v-show="currShowIndex==1">
            ...
            <div class="item" v-show="orderData.usedCount > 0" v-for="(item, uIndex) in usedList" :key="uIndex" :class="{ active: item.isShow }">
                <h4 @click="doSwitchShow(item)">{{ item.itemName }}券</h4>
                <div class="coupon" v-for="(coupon,index) in item.coupons" :key="index">
                    <div>券{{ index + 1 }}</div>
