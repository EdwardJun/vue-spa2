<!--拼团活动详情分享的页面-->
<style lang="sass">
    @import '../sass/page/groupBuyActShare.scss';
</style>
<template>
    <div class="page" id="group-buy-act-share-page">
        ...
        <div class="group-list-wrap" :class="{ full: groupBuyRecordList.length ==personalCount }">
            <div>{{ openUserName }}发起的拼团（<span>{{ groupBuyRecordList.length }}</span>/{{ personalCount }}）</div>
            <div>
                <div class="group-item" :class="{ opener: (p.userId == openUserId && i==0) }" v-for="(p,i) in groupBuyRecordList" :key="p.id">
                    <div :style="{ 'background-image': 'url(' + ( p.avatarUrl || global.defaultHeader ) +')' }"></div>
                    <div>{{ p.userName }}</div>
                    <div>{{ p.createTime }}</div>
                </div>
            </div>
            <div v-show="openGroupList.length>1" @click="doShowModal('showAllGroupsModal')">查看其他团</div>
        </div>
        <div class="club-pos">
            <div>{{ clubName }}</div>
            <div>
        ...
        <div class="btn-wrap">
            <div :class="{ disabled: isOver }" @click="doClickOpenGroup()" class="btn-left" v-if="!needHide">我要开团</div>
            <div :class="{ disabled: isOver || groupBuyRecordList.length ==personalCount }" @click="doJoinCurrGroup()" class="btn-right">
                <div>￥{{ actPrice | MoneyFilter }}<span>￥{{ itemPrice | MoneyFilter }}</span></div>
                <div>我要{{ needHide ? '购买' : '参团' }}</div>
            </div>
        </div>
        ...
        <div class="pop-modal all-groups-pop" :class="{ active: showAllGroupsModal }">
            <div class="center-wrap">
                <div>正在拼团</div>
                <div>
                    <div class="pop-group-item" v-for="(group, index) in openGroupList" :key="group.id" v-show="group.id != groupId">
                        ...
                        <div @click="doJoinGroup(group.id)" :class="{ 'disabled': group.personalCount - group.obtainedCount <=0 }">参团</div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.groupId = query.groupId
            if (!that.actId || !that.groupId) {
            ...
            // 获取支付渠道
            Global.getPayWay(that.global.clubId).then((res) => {
        ...
        methods: {
            hemaAuth () {
                ...
                if (that.global.userAgent.isWX && that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    Global.getSandHemaWechatAuth(location.href, that.global.clubId).then((res) => {
                        location.href = res
            ...
            init () {
                ...
                that.$http.get('../api/v2/user/group/buy/activity/detail', {params: {
                    activityId: that.actId, openGroupId: that.groupId
                }}).then(res => {
                    res = res.body
                    if (res.statusCode == 200) {
                        ...
                        if (res.groupBuyRecordList) {
                            that.groupBuyRecordList = res.groupBuyRecordList
                        }
                        that.openUserName = res.userName // 开团者
                        that.openUserId = res.userId
            ...
            initShareConfig () {
                ...
                if (global.userAgent.isWX) {
                    Global.shareConfig({
                        title: that.actName,
                        desc: '没有大动作，哪敢惊动你！' + that.itemName + '，原价' + ((that.itemPrice / 100).toFixed(2)) + '，现仅需' + ((that.actPrice / 100).toFixed(2)) + '，立刻点击参与！',
                        link: global.prefixPathname + '?#/' + that.clubId + '/groupBuyActShare?id=' + that.actId + '&techId=' + (that.techId || '') + '&userId=' + global.userId + '&groupId=' + query.groupId,
            ...
            doJoinCurrGroup () {
                ...
                if (that.groupBuyRecordList.length < that.personalCount) {
                    that.payType = 'join'
                    that.showPayModal = true
                    that.openGroupId = that.groupId
                    that.currJoinGroupObtainedCount = that.groupBuyRecordList.length
                } else {
            ...
            doClickPayBtn () {
                ...
                if (that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    location.reload()
                ...
                let sandSuccessUrl = that.global.prefixPathname + '#/' + that.global.clubId + '/groupBuyActJoinSuccess?id=' + that.actId + '&type=' + that.payType + '&count=' + that.count + '&clubId=' + that.clubId + '&techId=' + that.techId
                ...
                that.$http.post('../api/v2/wx/pay/group_buy/save', paramData).then(res => {
                    res = res.body
                    if (res.statusCode == 200) {
                        if (that.paymentChannel == 'sand') {
                            let respData = JSON.parse(res.respData)
                            location.href = respData.payUrl
                        }
                        if (ua.isWX) {
            ...
            afterPay (bizId) {
                let that = this
                let params = { id: that.actId, type: that.payType, count: that.count, clubId: that.clubId, bizId: bizId, techId: that.techId }
