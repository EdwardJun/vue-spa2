...
<template>
    <div class="page paid-coupon-page" id="paid-coupon-page">
        ...
        <div class="paid-coupon-bottom-wrap">
            ...
            <div v-if="(paymentChannel == 'sand' && sandOpenId && sandSubOpenId) || paymentChannel == 'gd'" :class="{ processing : inPaid , downline : isDownLine }" @click="doClickPayBtn()">{{ isDownLine ? "已下线" : ( inPaid ? "购买中..." : "立即购买")}}</div>
        </div>
    </div>
</template>
<script>
        ...
        methods: {
            hemaAuth () {
                ...
                if (that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                    Global.getSandHemaWechatAuth(location.href, that.couponData.clubId).then((res) => {
                        location.href = res
            ...
            init () {
                ...
                that.$http.get('../api/v2/club/redpacket/data', {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        that.couponData = res
                        // 获取支付渠道
                        Global.getPayWay(that.couponData.clubId).then((res) => {
            ...
            doClickPayBtn () { // 点击购买按钮
                ...
                if (!that.isDownLine) {
                    ...
                    if (!global.isLogin || !global.userTel) {
                    ...
                    } else if (that.paymentChannel == 'sand' && !that.sandOpenId && !that.sandSubOpenId) {
                        location.reload()
                        ...
                        let sandSuccessUrl = that.global.prefixPathname + '#/' + couponData.clubId + '/paidCouponDetail'
