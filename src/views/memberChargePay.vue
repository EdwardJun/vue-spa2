...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            beforeInit () {
                let that = this
                if (that.global.userAgent.isWX) { // 获取openId
            ...
            doPay () {
                ...
                let page = !global.isFollowed ? 'follow9358' : 'home'
                let sandSuccessUrl = that.global.prefixPathname + '#/' + that.global.clubId + '/' + page
                ...
                that.$http.post('../api/v2/wx/pay/recharge/save', paramData).then(function (res) {
                    res = res.body
                    if (res.statusCode == 200) {
                        if (that.amount > 0 && that.paymentChannel == 'sand') {
                            if (ua.isWX) {
